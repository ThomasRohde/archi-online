...
- `npm run lint` — eslint; `npm run docs:check` — validate wiki links
- `npm run ci:check` — full gate (version sync, docs, lint, typecheck, tests, audit, build); run before committing
...
- **Multi-model workspaces:** each open model is a session in `src/model/workspace.ts` with its own `ModelStore`; ops take an optional target store that defaults to the active session. UI rendered for a specific session (tree rows, canvas, context menus) must pass its session's store explicitly — relying on the globally active store risks silent wrong-model mutations (see `docs/reviews/multi-model-workspace-review.md`).
...
- `src/model/` and `src/scripting/` must stay free of React imports (DOMParser in `io/` is the only DOM use). The stores are vanilla zustand; the React bindings (`useStore`, `useWorkspaceStore`, `ModelStoreProvider`) live in `src/ui/store-hooks.tsx`. Imperative code uses `getActiveModelStore()` / `workspaceStore` directly.
- Dockview panels sync with the store in `src/ui/DockLayout.tsx` behind a module-level `syncing` guard; view panels have ids `view:<sessionId>:<viewId>` (both URL-encoded — see `src/ui/dock/layout-config.tsx`). Don't persist layout while a group is maximized.
