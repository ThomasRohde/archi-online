  ...
  const visit = (current) => {
    for (const entry of readdirSync(current, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        files[rel] = strToU8(readFileSync(path, 'utf8'));
      }
    }
  };
  visit(dir);
...
for (const name of packages) {
  ...
  if (!statSync(manifestPath).isFile()) throw new Error(`${name} is missing manifest.json`);
  ...
  if (!statSync(mainPath).isFile()) throw new Error(`${name} is missing ${manifest.main}`);
